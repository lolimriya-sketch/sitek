  ...
  const renderedElements = currentScene
    ? currentScene.elements
        ...
        .map((element, idx) => {
          ...
          if (idx === 0) {
            // eslint-disable-next-line no-console
            console.debug(python3 - << 'END'
import json, os
from datetime import datetime, timezone, timedelta

# Папка з історією VS Code
history_dir = "/home/codespace/.vscode-remote/data/User/History/"
# Папка проекту
project_dir = "/workspaces/site_lms/"

# Час відновлення: 9:40 локальний час (UTC = локальний -2 години)
restore_time = datetime.now(timezone.utc).replace(hour=7, minute=40, second=0, microsecond=0) # 9:40 локального +02:00

# Зберігаємо останню версію кожного файлу
latest_versions = {}
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        for entry in entries:
                            ...
                            if filename and content and timestamp:
                                entry_time = datetime.fromtimestamp(timestamp / 1000, timezone.utc)
                                # Беремо останню версію до restore_time
                                if entry_time <= restore_time:
                                    if filename not in latest_versions or entry_time > latest_versions[filename][0]:
                                        latest_versions[filename] = (entry_time, content)
                except Exception as e:
                    print(f"Не вдалося обробити {file_path}: {e}")

# Відновлюємо файли у проекті
for filename, (ts, content) in latest_versions.items():
    file_out_path = os.path.join(project_dir, filename.replace("/", os.sep))
    os.makedirs(os.path.dirname(file_out_path), exist_ok=True)
    with open(file_out_path, "w", encoding="utf-8") as f:
        f.write(content)

print(f"Відновлення завершено. Відновлено {len(latest_versions)} файлів станом на 9:40.")
END
"[editor] first element computed", { id: element.id, left, top, width, height, canvasW, canvasH })
          }
