            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {groupedProgress.length > 0
                  ? (() => {
                      // compute average over entries that have a valid duration
                      const durations = groupedProgress.map((g) => getAttemptDurationSeconds(g.latestAttempt)).filter((d) => d > 0)
                      if (durations.length === 0) return "0хв"
                      const avg = Math.floor(durations.reduce((s, d) => s + d, 0) / durations.length)
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {groupedProgress.length > 0
                  ? (() => {
                      const exits = groupedProgress.map((g) => Number(g.latestAttempt?.tabExits) || 0).filter((n) => n >= 0)
                      if (exits.length === 0) return 0
                      return Math.floor(exits.reduce((s, n) => s + n, 0) / exits.length)
                    })()
                  : 0}
              </p>
            </div>
                    ...
                    <Card key={gp.userId} className="p-4">
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{formatDuration(getAttemptDurationSeconds(latest))}</p>
                        </div>
